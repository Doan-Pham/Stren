...
class ExercisesViewModelTest {
    ...
    fun uiState_exercisesLoaded_isLoadComplete() = runTest {
        // Need this since StateFlow created with stateIn doesn't update unless there's collectors
        val collectJob =
            launch(UnconfinedTestDispatcher(testScheduler)) { viewModel.uiState.collect() }
        runCurrent()
